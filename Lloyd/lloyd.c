  ...
  switch(signum){
    case SIGINT:
      ...
      for(int i = 0; i < numDades; i++){
        free(estacions[i].nomEstacio);
        free(infoAcumulada[i].nomEstacio);
      }
      free(estacions);
      free(infoAcumulada);
      free(numDades);
...
int main(){
  ...
  //Reassignem signals
  signalhandler(SIGINT, signalhandler);
  ...
  while (1) {
    ...
    if (numEstacions != 0) {
      ...
      if (estacio == -1){
        ...
        estacions = (infoLloyd *) realloc(sizeof(infoLloyd)*numEstacions); //Reservem espai per la nova estacio
        infoAcumulada = (infoLloyd *) realloc(sizeof(infoLloyd) * numEstacions); //Reservem espai per la nova estacio a l'acumulat
        numDades = (int *) realloc(sizeof(int)*numEstacions); //Reservo espai per el nombre de dades/estacio
        ...
        numDades[estacio]++;
        infoAcumulada[estacio] = (infoLloyd *) realloc(sizeof(infoLloyd)*numDades[estacio]);
